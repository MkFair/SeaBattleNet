...
bool CheckSequence(int quantity_of_deck, std::vector<std::vector<int>> my_ship, IsSuccessively_t IsSuccessivelyF1, IsSuccessively_t IsSuccessivelyF2)//ÔÓ‰ˇ‰ ÎË ÍÎÂÚÓ˜ÍË?‡„ÛÏÂÌÚ˚: ÒÌ‡˜‡Î‡ ÔÓÎÂ Ë Â„Ó ‡ÁÏÂ˚. ÔÓÚÓÏ ÍÓÓ‰ËÌ‡Ú˚ ÍÓ‡·Îˇ
{
	...
	for (int i = 0; i < quantity_of_ships; ++i)// a loop of entering ships  ˆËÍÎ ‚‚Â‰ÂÌËˇ ÍÓ‡·ÎÂÈ
	{
		...
		do
		{
			for (int i = 0; i < quantity_of_decks; ++i)// œ–Œ’Œƒ»Ã œŒ  ¿∆ƒŒ… œ¿À”¡≈
			{
				...
				//œ–Œ¬≈– ¿ Õ≈“ À» –ﬂƒŒÃ ƒ–”√»’  Œ–¿¡À≈…. 
				if (field[row_number][column_number] == 'k' || IsPartOfShipF(ship, row_number, column_number)  )//ÂÒÎË ÛÊÂ Ú‡Ï ÒÚÓËÚ ÍÓ‡·Î¸ ËÎË ÂÒÎË ˇ‰ÓÏ ÛÊÂ ÒÚÓˇÎ ÍÓ‡·Î¸
				{
		...
		} while(CheckSequence(quantity_of_decks, ship, AreCellsArrangedHorizontallySuccessively, AreCellsArrangedVerticallySuccessively)); //ÔÓ‚ÂÍ‡ œŒÀÕŒ÷≈ÕÕ€… À»  Œ–¿¡À‹
